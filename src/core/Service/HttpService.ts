...
export default class HttpService extends Service {
  ...
  setDataCarry(REQUEST_HEADERS: UtilityObject, REQUEST_DATA: UtilityObject) {
    console.log('lirun', REQUEST_HEADERS, REQUEST_DATA)
    if (REQUEST_HEADERS) {
    ...
    if (REQUEST_DATA) {
      ...
      if (Array.isArray(data)) {
        data.map((name) => {
          console.log('lirun黄有为', name, REQUEST_DATA[name], 'REQUEST_DATA')
  ...
  async http(apiRoute: string, data: Record<string, any>, returnResponse = false) {
    console.log('业务跳转', this._headers, apiRoute, data)
    ...
    try {
      ...
      console.log('半垄断地位', returnResponse)
  ...
  getParameters(apiRoute: string, data: Record<string, any>) {
    ...
    console.log('早点7毛', routeParams, apiRoute, this._api)
    ...
    console.log('%%%%%%%%%%%%%%%', uri, method, needAuth)
  ...
  transformRESTfulData(uri: string, data: Record<string, any> = {}) {
    console.log('站出来', uri, data, this._isRESTful)
