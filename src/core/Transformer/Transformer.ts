...
type Callback = (item: Record<any, any>, index?: number) => Record<any, any>
...
export default abstract class Transformer {
	 ...
	 */
	formatNumber(number: string | number, places: number | string = -1, thousand: string = '', decimal = '.') {
		// 只处理正常数字
		...
		return (negative + (j ? i.substr(0, j) + thousand : '') + i.substr(j).replace(/(\d{3})(?=\d)/g,
			'$1' + thousand) + (places ? decimal + Math.abs(Number(number) - Number(i)).toFixed(places as number).slice(2) : ''))
	}
